...
const getMyOrders = async (req: Request, res: Response) => {
  try {
    //populate = This tells Mongoose to populate the restaurant field in each of the found orders with the actual data from the restaurants collection (or schema).
...
const createCheckoutSession = async (req: Request, res: Response) => {
  try {
    ...
    //LineItems = the left hand side stuff you see about our order and subtotal and stuff.
    ...
    const session = await createSession(
      lineItems,
      newOrder._id.toString(), //even thou we haven't saved it to DB yet, mongoose have created an ID for us to use.
